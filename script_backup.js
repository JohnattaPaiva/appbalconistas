...
function editarProdutoTabela(produtoId) {
  ...
  try {
    ...
    // Primeiro, mostrar a seção de produtos
    mostrarSecao('produtos');
    
    // Exibir o botão "Cadastrar Produto" se necessário
    exibirBotaoCadastrarProduto();
    ...
    setTimeout(() => {
      try {
        ...
        const botaoCadastrar = document.getElementById('btn-cadastrar-produto');
        if (botaoCadastrar) {
          console.log('Clicando no botão cadastrar produto...');
        ...
        setTimeout(() => {
          ...
          if (!listaElement || !formElement) {
            ...
            toastErro('Erro', 'Erro na interface! Elementos não encontrados.');
            return;
          }
          ...
          // Aguardar mais um pouco para garantir que o formulário está visível
          setTimeout(() => {
          ...
          if (pontosInput) {
            pontosInput.value = produto.pontos || '';
            console.log('Pontos preenchidos:', pontosInput.value);
          } else {
            console.error('Campo pontos não encontrado');
          }
          
          if (categoriaSelect) {
            categoriaSelect.value = produto.categoria || '';
            console.log('Categoria preenchida:', categoriaSelect.value);
          } else {
            console.error('Campo categoria não encontrado');
          }
          
          if (ativoSelect) {
            ativoSelect.value = produto.ativo ? 'true' : 'false';
            console.log('Status preenchido:', ativoSelect.value);
          } else {
          ...
          if (titleElement) {
            titleElement.textContent = '✏️ Editar Produto';
          ...
          if (form) {
            ...
            // Configurar novo handler para edição
            form.onsubmit = function(event) {
              event.preventDefault();
              console.log('Formulário submetido para edição');
              salvarEdicaoProduto(produtoId);
            };
            console.log('Formulário configurado para edição');
          }
          
          // Processar cores do produto
          if (produto.cores && produto.cores.length > 0) {
            console.log('Processando cores:', produto.cores);
            ...
            console.log('Cores a serem selecionadas:', coresSelecionadasIds);
          ...
          if (produto.tamanhos && produto.tamanhos.length > 0) {
            console.log('Processando tamanhos:', produto.tamanhos);
            ...
            document.querySelectorAll('input[name="tamanhos"]').forEach(checkbox => {
              if (produto.tamanhos.includes(checkbox.value)) {
                checkbox.checked = true;
                console.log('Tamanho selecionado:', checkbox.value);
              }
            });
          }
          ...
          if (produto.imagens && produto.imagens.length > 0) {
            console.log('Processando imagens:', produto.imagens.length);
            // Aqui você pode adicionar lógica para carregar as imagens se necessário
          }
          ...
          }, 300); // Aguardar 300ms para garantir que o formulário está renderizado
          
        } catch (error) {
          console.error('Erro ao configurar formulário:', error);
          toastErro('Erro', 'Erro ao configurar formulário: ' + error.message);
        }
      }, 200); // Aguardar 200ms para o formulário abrir
      
    } catch (error) {
      console.error('Erro ao abrir formulário:', error);
      toastErro('Erro', 'Erro ao abrir formulário: ' + error.message);
    }
  }, 100); // Aguardar 100ms para garantir que a seção foi carregada
...
function editarProduto(produtoId) {
  ...
  document.getElementById('produto-ativo').value = produto.ativo.toString();
  
  // Marca as cores selecionadas
  document.querySelectorAll('input[name="cores"]').forEach(checkbox => {
    checkbox.checked = produto.cores.includes(checkbox.value);
  });
  
  // Marca os tamanhos selecionados
  document.querySelectorAll('input[name="tamanhos"]').forEach(checkbox => {
    checkbox.checked = produto.tamanhos.includes(checkbox.value);
  });
  
  // Altera o botão de submit para edição
  const form = document.getElementById('form-produto');
  form.onsubmit = function(event) {
    event.preventDefault();
    salvarEdicaoProduto(produtoId);
